...
def get_free_car_ids(origin, start_time, end_time):
    ...
    booked_cars = Schedule.objects.exclude(start_time__gt=end_time).\
        exclude(end_time__lt=start_time).values("car_id")

    free_cars = Car.objects.exclude(id__in=booked_cars).filter(id__in=available_at_start_loc)
...
def get_inventory_at_date(branch, cutoff_time):
    ...
    try:
        schedules = Schedule.objects.filter(
            Q(end_time__gt=start_time), Q(start_time__lt=cutoff_time),
    ...
    for s in list(schedules):
        ...
        if s.destination_branch == branch and s.car_id not in current_inventory \
                and s.end_time < cutoff_time:
            current_inventory.append(s.car_id)
        # if the start branch is ours, the car shooould be there.
        elif s.origin_branch == branch and s.destination_branch != branch and\
                s.car_id in current_inventory:
